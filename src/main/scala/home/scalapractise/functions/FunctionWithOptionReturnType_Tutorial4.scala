...
object FunctionWithOptionReturnType_Tutorial4 extends App {
  ...
  def dailyCouponCode(): Option[String] = {
    ...
    Option(couponFromDb).filter(!_.isEmpty)
  }

  /* to understand what is _.nonEmpty follow these things
  scala> var v:String = _
         v: String = null

  scala> Option(v).filter(_.nonEmpty)
  res15: Option[String] = None

  scala> Option(v).filter(!_.isEmpty)
