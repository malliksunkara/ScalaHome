...
object FunctionWithOptionParameters_Tutorial3 extends App {
  ...
  def calculateDonutCostWithDefaultOptionValue(donutName: String, quantity: Int, couponCode: Option[String] = None): Double = {
    ...
    couponCode match{
      case Some(coupon) =>
        println("some(coupon) is executed")
        val discount = 0.1 // Let's simulate a 10% discount
        val totalCost = 2.50 * quantity * (1 - discount)
        totalCost

      case _ =>
         println("case _ is exeucted")
        2.50 * quantity
      case None =>
        println("case None is exeucted")
        2.50 * quantity
  ...
  // from the below statements we can say that case _ or case None ( case _ will exeucte from calling)
  println(s"""Total cost with discount = ${calculateDonutCostWithDefaultOptionValue("Glazed Donut", 5, Some("COUPON_1234"))}""")
  println(s"""Total cost with discount = ${calculateDonutCostWithDefaultOptionValue("Glazed Donut", 5, None)}""")
